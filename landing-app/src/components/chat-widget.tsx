...
const welcomeMessage = `Подберу код КТРУ за минуту.

Напишите, что нужно купить — если не хватит данных, уточню и предложу готовые варианты.`;
...
export const ChatWidget = ({ mode = "drawer", defaultOpen = false, hideFloatingButton = false }: ChatWidgetProps) => {
  ...
  const thinkingPhrases = useMemo<string[]>(
    () => [
      "Подбираем код…",
      "Бежим в справочник…",
      "Сверяем варианты КТРУ…",
      "Достаём чек‑листы…",
      "Ищем лучшие совпадения…",
      "Проверяем параметры…",
      "Уточняем формулировку запроса…",
      "Формируем параметры…",
    ],
    []
  );
